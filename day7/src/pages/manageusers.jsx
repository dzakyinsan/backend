...
function ManageAdmin() {
  ...
  const renderusers=()=>{
   return datausers.map((val,index)=>{
      ...
      <tr key={index}>
        ...
        <td>{val.username}</td>
        <td><img src={`${APIURLimage+val.image}`} alt={index} width='100px' height='100px'/></td>
        <td>{val.email}</td>
        <td>{val.phone}</td>
        <td>{val.usia}</td>
        {
  ...
  const Deletedata=()=>{
    var iduser=datausersdelete.id
      Axios.delete(`${APIURL}user/users/${iduser}`)
      .then(()=>{
        Axios.get(`${APIURL}user/users`)
        .then((res)=>{
          setdatausers(res.data.datauser)
  ...
  return (
    <Fragment>
      ...
      <Modal title={`edit data ${datausersedit.username}`} toggle={toggle} modal={modal} actionfunc={Updatedata} btnTitle='save'>
      ...
      <Modal title='delete data' toggle={toggledelete} modal={modaldelete} actionfunc={Deletedata} btnTitle='delete'>
        fsdasd delete
      </Modal>
      <Table striped>
        <thead>
